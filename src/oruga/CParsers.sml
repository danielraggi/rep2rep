...
signature CPARSERS =
sig
  val parseProbSys : string -> construction;
end
...
struct
  ...
  fun parseProbSys s =
      let val dContent = Document.read "bTreeBayes"
          ...
          val btree1 = getTopConstruction (Seq.hd (Transfer.structureTransfer (SOME 100) false NONE state0))
          ...
          val btree2 = getTopConstruction (Seq.hd (Transfer.structureTransfer (SOME 100) false NONE state1))
          ...
          val btree3 = getTopConstruction (Seq.hd (Transfer.structureTransfer (SOME 100) false NONE state2))
          ...
          val btree4 = getTopConstruction (Seq.hd (Transfer.structureTransfer (SOME 50) false NONE state3))
          ...
          val btree5 = getTopConstruction (Seq.hd (Transfer.structureTransfer (SOME 100) false NONE state4))
          ...
          val btree6 = getTopConstruction (Seq.hd (Transfer.structureTransfer (SOME 50) false NONE state5))
          ...
          val btree7 = getTopConstruction (Seq.hd (Transfer.structureTransfer (SOME 100) false NONE state6))
          ...
          val btree8 = getTopConstruction (Seq.hd (Transfer.structureTransfer (SOME 50) false NONE state7))
          val _ = print (Construction.toString btree8 ^ "\n\n")
          val construct8 = Construction.constructOf btree8
          val goal8 = Document.parseConstruction interBTreeBayesConSpecData (":metaTrue <- SYS[" ^ (CSpace.stringOfToken construct8) ^ ",t':btree]")
          val state8 = Transfer.initState btreeConSpecData bayesConSpecData interBTreeBayesConSpecData false KB btree8 goal8
      in getTopConstruction (Seq.hd (Transfer.structureTransfer (SOME 200) false NONE state8))
      end
end
